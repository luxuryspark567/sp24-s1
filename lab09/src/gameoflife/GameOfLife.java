...
public class GameOfLife {
    ...
    public TETile[][] nextGeneration(TETile[][] tiles) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int l = -1; l <= 1; l++) {
                        if (!(i == 0 && l == 0) && (x + i) >= 0 && (x + i) < width && (y + l) >= 0 && (y + l) < height) {
                            if (tiles[x + i][y + l] == Tileset.CELL) {
                                neighborsL++;
                            }
                        }
                    }
    ...
    public void saveBoard() {
        ...
        for (int y = height - 1; y >= 0; y--) {
            for (int x = 0; x < width; x++) {
                char tile = currentState[x][y] == Tileset.CELL ? '1' : '0';
                builder.append(tile);
            }
            builder.append("\n");
        }
    ...
    public TETile[][] loadBoard(String filename) {
        ...
        TETile[][] lTiles = new TETile[width][height];
        for (int y = 0; y < height; y++) {
            String line = lines[y + 1];
            for (int x = 0; x < width; x++) {
                char cell = line.charAt(x);
                lTiles[x][height - y - 1] = cell == '1' ? Tileset.CELL : Tileset.NOTHING;
            }
        }
        this.width = width;
        this.height = height;
